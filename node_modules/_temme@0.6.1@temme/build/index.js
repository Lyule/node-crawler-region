(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		exports["temme"] = factory(require("cheerio"));
	else
		root["temme"] = factory(root["cheerio"]);
...
function check(selector) {
    ...
    if (selector.type === 'self-selector') {
        throw new Error(exports.msg.selfSelectorAtTopLevel());
    }
...
function commonCheck(selector) {
    if (selector.type === 'normal-selector') {
        ...
        if (hasLeadingCapture) {
            throw new Error(exports.msg.hasLeadingAttributeCapture());
        }
    }
}
function checkChild(selector) {
    ...
    if (selector.type === 'snippet-define') {
        throw new Error(exports.msg.snippetDefineNotAtTopLevel(selector.name));
    }
    else if (selector.type === 'normal-selector') {
        for (const child of selector.children) {
            checkChild(child);
...
const check_1 = __webpack_require__(0);
        ...
        for (const qualifier of section.qualifiers) {
            if (qualifier.type === 'id-qualifier') {
                ...
                if (operator == null && value == null) {
                    ...
                    invariant(operator === '=', check_1.msg.valueCaptureWithOtherOperator());
                }
                else {
...
Object.defineProperty(exports, "__esModule", { value: true });
exports.defaultCaptureKey = '@@default-capture@@';
exports.specialFilterNames = ['html', 'outerHTML', 'node', 'text'];
exports.universalSelector = '*';
...
class CaptureResult {
    ...
    add(key, value, filterList) {
        ...
        if (!(value == null || utils_1.isEmptyObject(value))) {
            this.result[key] = value;
        }
    }
    forceAdd(key, value, filterList) {
        if (this.failed) {
            return;
        }
        if (filterList) {
    ...
    mergeWithFailPropagation(other) {
        if (other.isFailed()) {
            this.setFailed();
        }
        else {
    ...
    get() {
        ...
        if (returnVal.hasOwnProperty(constants_1.defaultCaptureKey)) {
            returnVal = this.result[constants_1.defaultCaptureKey];
        }
        if (utils_1.isEmptyObject(returnVal)) {
            returnVal = null;
        }
        return returnVal;
    }
    ...
    applyFilter(value, filter) {
        if (filter.name in this.filterFnMap) {
            ...
            throw new Error(check_1.msg.invalidFilter(filter.name));
        }
    }
    applyFilterList(initValue, filterList) {
        return filterList.reduce((value, filter) => {
            if (filter.isArrayFilter) {
                invariant(Array.isArray(value), check_1.msg.arrayFilterAppliedToNonArrayValue(filter.name));
...
function find(result, node, args) {
    ...
    if (args.length === 2) {
        ...
        invariant(typeof before === 'string' && utils_1.isCapture(after) || utils_1.isCapture(before) && typeof after === 'string', invalidArgs);
        ...
        invariant(typeof before === 'string' && utils_1.isCapture(capture) && typeof after === 'string', invalidArgs);
        const i = s.indexOf(before);
        if (i === -1) {
            result.setFailed();
        }
...
exports.contentFunctions = {
    get(name) {
        if (map.has(name)) {
            ...
            throw new Error(check_1.msg.invalidContentFunction(name));
        }
    },
    set(name, fn) {
        map.set(name, fn);
...
exports.contentFunctions.set('find', find);
...
Object.defineProperty(exports, "__esModule", { value: true });
const cheerio = __webpack_require__(10);
exports.cheerio = cheerio;
const invariant = __webpack_require__(1);
const filters_1 = __webpack_require__(6);
const contentFunctions_1 = __webpack_require__(5);
const check_1 = __webpack_require__(0);
const CaptureResult_1 = __webpack_require__(4);
const constants_1 = __webpack_require__(3);
const utils_1 = __webpack_require__(2);
let temmeParser;
exports.temmeParser = temmeParser;
if (true) {
    exports.temmeParser = temmeParser = __webpack_require__(11);
}
else {
    const fs = require('fs');
    const pegjs = require('pegjs');
    const source = fs.readFileSync('./src/grammar.pegjs', 'utf8');
    exports.temmeParser = temmeParser = pegjs.generate(source);
}
function temme(html, selector, extraFilters = {}) {
    ...
    else if (utils_1.isCheerioStatic(html)) {
        $ = html;
    }
    else {
        $ = cheerio.load(html);
    ...
    rootSelector.forEach(check_1.check);
    ...
    function helper(cntCheerio, selectorArray) {
        const result = new CaptureResult_1.CaptureResult(filterFnMap);
        // First pass: process SnippetDefine and FilterDefine
        for (const selector of selectorArray) {
            if (selector.type === 'snippet-define') {
                invariant(!snippetsMap.has(selector.name), check_1.msg.snippetAlreadyDefined(selector.name));
                ...
                filterFnMap[name] = new Function(...argNames, code);
            }
        }
        // Second pass: process match and capture
        for (const selector of expandSnippets(selectorArray)) {
            if (selector.type === 'normal-selector') {
                const cssSelector = utils_1.makeNormalCssSelector(selector.sections);
                ...
                const cssSelector = utils_1.makeNormalCssSelector([selector.section]);
    ...
    function expandSnippets(selectorArray, expanded = []) {
        ...
        for (const selector of selectorArray) {
            if (selector.type === 'snippet-expand') {
                invariant(snippetsMap.has(selector.name), check_1.msg.snippetNotDefined(selector.name));
                const snippet = snippetsMap.get(selector.name);
                const nextExpanded = expanded.concat(snippet.name);
                invariant(!expanded.includes(snippet.name), check_1.msg.circularSnippetExpansion(nextExpanded));
    ...
    function capture(node, selector) {
        const result = new CaptureResult_1.CaptureResult(filterFnMap);
        if (selector.type === 'normal-selector') {
            ...
            result.mergeWithFailPropagation(captureAttributes(node, qualifiers.filter(utils_1.isAttributeQualifier)));
    ...
    function captureAttributes(node, attributeQualifiers) {
        const result = new CaptureResult_1.CaptureResult(filterFnMap);
    ...
    function captureContent(node, content) {
        ...
        for (const part of content) {
            if (part.type === 'capture') {
                ...
                const normalFilterList = constants_1.specialFilterNames.includes(firstFilterName)
                    ? filterList.slice(1)
                    : filterList;
                ...
                contentFunctions_1.contentFunctions.get(funcName)(result, node, args);
            }
        }
        return result;
    }
...
const temme_1 = __webpack_require__(7);
exports.default = temme_1.default;
...
exports.DummyValue325353 = '242gascdfe';
...
function peg$parse(input, options) {
  ...
  var peg$FAILED = {},
      ...
      peg$c4 = function(section, content) {
          return {
            type: 'self-selector',
      ...
      peg$c7 = function(sections, content) {
          return {
            type: 'normal-selector',
            sections,
            content,
      ...
      peg$c18 = function(name, argNames, code) {
          return {
            type: 'filter-define',
            name,
            argNames,
      ...
      peg$c28 = function(capture, value) {
      ...
      peg$c43 = function(name, filterList) {
          return { name, filterList }
        },
      peg$c44 = function(captureKey) {
      ...
      peg$c55 = peg$otherExpectation("normal css selector"),
      peg$c56 = peg$otherExpectation("css-selector-section-seperator"),
      peg$c57 = peg$otherExpectation("section"),
      peg$c58 = function(c) { return c },
      peg$c59 = "*",
      peg$c60 = peg$literalExpectation("*", false),
      peg$c61 = function(combinator, element, qualifiers) {
          return {
      ...
      peg$c62 = function(combinator, qualifiers) {
          return {
            combinator: combinator || ' ',
            element: '*',
            qualifiers: flatten(qualifiers),
      ...
      peg$c63 = peg$otherExpectation("css-selector-qualifier"),
      peg$c64 = peg$otherExpectation("css-selector-id-qualifier"),
      peg$c65 = "#",
      ...
      peg$c68 = peg$otherExpectation("css-selector-class-qualifier"),
      peg$c69 = ".",
      peg$c70 = peg$literalExpectation(".", false),
      peg$c71 = function(className) {
          return {
            type: 'class-qualifier',
            className,
          }
        },
      peg$c72 = peg$otherExpectation("css-selector-attribute-qualifier"),
      peg$c73 = "[",
      peg$c74 = peg$literalExpectation("[", false),
      peg$c75 = function(head, part) { return part },
      peg$c76 = "]",
      peg$c77 = peg$literalExpectation("]", false),
      peg$c78 = function(head, tail) {
          return [head].concat(tail)
        },
      peg$c79 = peg$otherExpectation("css-selector-attribute-qualifier-part"),
      peg$c80 = function(attribute, operator, value) {
          return {
      ...
      peg$c82 = peg$otherExpectation("css-selector-pseudo-qualifier"),
      peg$c83 = ":",
      peg$c84 = peg$literalExpectation(":", false),
      peg$c85 = /^[^)]/,
      peg$c86 = peg$classExpectation([")"], true, false),
      peg$c87 = function(name, content) {
          return {
      ...
      peg$c89 = function(capture) {
          return {
            type: 'capture',
            capture,
          }
        },
      peg$c90 = function(funcName) {
          return {
            type: 'call',
            funcName,
            args: [],
      ...
      peg$c91 = function(funcName, head, tail) {
          return {
            type: 'call',
            funcName,
      ...
      peg$c94 = function(filterList) {
          return { name: defaultCaptureKey, filterList }
        },
      peg$c95 = "-",
      peg$c96 = peg$literalExpectation("-", false),
      peg$c97 = /^[_a-z]/i,
      peg$c98 = peg$classExpectation(["_", ["a", "z"]], false, true),
      peg$c99 = /^[_a-z0-9\-]/i,
      peg$c100 = peg$classExpectation(["_", ["a", "z"], ["0", "9"], "-"], false, true),
      peg$c101 = /^[\x80-\uFFFF]/,
      peg$c102 = peg$classExpectation([["\x80", "\uFFFF"]], false, false),
      ...
      peg$c179 = function() { return "\0"; },
      peg$c180 = "b",
      peg$c181 = peg$literalExpectation("b", false),
      peg$c182 = function() { return "\b"; },
      peg$c183 = "f",
      peg$c184 = peg$literalExpectation("f", false),
      peg$c185 = function() { return "\f"; },
      peg$c186 = "n",
      peg$c187 = peg$literalExpectation("n", false),
      peg$c188 = function() { return "\n"; },
      peg$c189 = "r",
      peg$c190 = peg$literalExpectation("r", false),
      peg$c191 = function() { return "\r"; },
      peg$c192 = "t",
      peg$c193 = peg$literalExpectation("t", false),
      peg$c194 = function() { return "\t"; },
      peg$c195 = "v",
      peg$c196 = peg$literalExpectation("v", false),
      peg$c197 = function() { return "\v"; },
      peg$c198 = "x",
      peg$c199 = peg$literalExpectation("x", false),
      peg$c200 = "u",
      peg$c201 = peg$literalExpectation("u", false),
      peg$c202 = peg$otherExpectation("code block"),
      peg$c203 = function(code) { return code },
      peg$c204 = function() { error("Unbalanced brace.") },
      peg$c205 = /^[{}]/,
  ...
  function peg$parseSelfSelector() {
    var s0, s1, s2, s3, s4;
    ...
    if (s1 !== peg$FAILED) {
      ...
      if (s2 !== peg$FAILED) {
        ...
        if (s3 !== peg$FAILED) {
          s4 = peg$parse__();
          if (s4 !== peg$FAILED) {
            s5 = peg$parseFunctionDefineArgNames();
            if (s5 !== peg$FAILED) {
              s6 = peg$parse__();
              if (s6 !== peg$FAILED) {
  ...
  function peg$parseFunctionDefineArgNames() {
    ...
    if (s0 === peg$FAILED) {
      ...
      if (s1 !== peg$FAILED) {
        s2 = peg$parse__();
        if (s2 !== peg$FAILED) {
          s3 = peg$parseRestIdentifierName();
          if (s3 !== peg$FAILED) {
            s4 = [];
            s5 = peg$currPos;
            s6 = peg$parse__();
            if (s6 !== peg$FAILED) {
              ...
              if (s7 !== peg$FAILED) {
                s8 = peg$parse__();
                if (s8 !== peg$FAILED) {
                  s9 = peg$parseRestIdentifierName();
                  if (s9 !== peg$FAILED) {
                    s6 = [s6, s7, s8, s9];
                    s5 = s6;
                  } else {
  ...
  function peg$parseRestIdentifierName() {
    var s0, s1, s2, s3, s4;
      ...
      if (s1 !== peg$FAILED) {
        ...
        if (s2 !== peg$FAILED) {
          ...
          if (s3 !== peg$FAILED) {
            ...
            if (s4 !== peg$FAILED) {
              s5 = peg$parseOptionalExtraSeperator();
              if (s5 !== peg$FAILED) {
                s6 = peg$parse__();
                if (s6 !== peg$FAILED) {
                  if (input.charCodeAt(peg$currPos) === 125) {
  ...
  function peg$parseContentPartArg() {
    var s0;
